...
class Admin(user):
    register_schema_list = \
        [
            'player','player_record', 'agent', 'owner', 'team', 'director', 'user', 'player_agent',
    ...
    def register_data(self, cursor):
        print('**** Register Data ****')
        for i in range(0, len(Admin.register_schema_list)):
            print(f'{i + 1}. {Admin.register_schema_list[i]}')

        database_schema = input()

        if database_schema not in Admin.register_schema_list:
            super().error()
            return
    ...
    def read_data(self, cursor):
        print('**** Read Data ****')
        for i in range(0, len(Admin.read_schema_list)):
            print(f'{i + 1}. {Admin.read_schema_list[i]}')

        database_schema = input()

        if database_schema not in Admin.read_schema_list:
            super().error()
            return
